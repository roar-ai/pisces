...
class EulerSolver:
    ...
    def euler_style_multiphase_pred_last_step(
        self,
        sample,
        model_pred,
        timestep_index,
        ...
        last_valid_index = valid_indices_mask.flip(dims=[1]).long().argmax(
            dim=1)
        last_valid_index = inference_indices.size(0) - 1 - last_valid_index
        timestep_index_end = torch.zeros_like(last_valid_index, dtype=inference_indices.dtype, device=inference_indices.device)
