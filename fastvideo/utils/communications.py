...
#     Each rank starts with a different element, and at each step, all ranks get the same element.
#     """
#     world_size = dist.get_world_size(nccl_info.group)
#     rank = dist.get_rank(nccl_info.group)

#     # Ensure input is a list
#     if not isinstance(input_, list):
#         raise TypeError("Input to broadcast must be a list.")

#     # Ensure all ranks start with the same list order
#     dist.broadcast_object_list(input_, src=0, group=nccl_info.group)

#     for i in range(len(input_)):
#         broadcasted_item = [input_[i]] * world_size
#         dist.broadcast_object_list(broadcasted_item, src=0, group=nccl_info.group)
#         yield broadcasted_item[rank]


# def broadcast_caption(caption: List[Any]):
#     """
#     Broadcasts caption data across all ranks to ensure synchronization.
#     """
...
def broadcast_custom(input_: List[Any]):
    ...
    all ranks will have the full list of elements from all ranks.
    """
    world_size = dist.get_world_size(nccl_info.group)
    rank = dist.get_rank(nccl_info.group)
    ...
    # Gather all elements from all ranks
    gathered_data = [None] * world_size
    dist.all_gather_object(gathered_data, input_[0], group=nccl_info.group)
...
def prepare_sequence_parallel_data(
    ...
    def prepare(
        ...
        caption = broadcast_caption(caption)
        return (
            hidden_states,
            encoder_hidden_states,
            attention_mask,
...
def sp_parallel_dataloader_wrapper(
    ...
    while True:
        for data_item in dataloader:
            latents, cond, attn_mask, cond_mask, caption = data_item
            ...
            if frame == 1:
                yield latents, cond, attn_mask, cond_mask, caption
            else:
                latents, cond, attn_mask, cond_mask, caption = (
                ...
                for iter in range(train_batch_size * sp_size // train_sp_batch_size):
                    ...
                    each_caption = caption[st_idx:ed_idx]
                    yield (
